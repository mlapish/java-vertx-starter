podTemplate(label: 'buildpod', inheritFrom: 'maven', privileged: true, serviceAccount: 'jenkins', cloud: 'openshift', containers: [
    ...
    node('buildpod') {    
      ...
      stage('Docker Test') {
            sh "sudo docker login http://100.65.143.111:5000 -u jenkins -p `oc sa get-token jenkins`"
            sh "sudo docker pull 100.65.143.111:5000/quota-test/java-vertx-starter"
      }
   }
}
